...
def user(username):
    ...
    try:
        ...
        posts = db.posts.find({'username': username}).sort("date", -1).limit(20)

        return render_template('user.html', user_info=user_info, status=status, posts=posts)
...
def sign_up():
    ...
    doc = {
        ...
        "profile_pic": "",                                          
        "profile_pic_real": "profile_pics/profile", 
        "profile_info": ""                                          
    }
...
def posting():
    ...
    try:
        ...
        posts = db.posts.find({'username': user_info['username']}).sort("date", -1).limit(20)
        return render_template('user.html', user_info=user_info, user_posts=posts)
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
